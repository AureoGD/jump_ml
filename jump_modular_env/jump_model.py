...
class JumpModel:
    ...
    def __init__(self, _robot_states, N=10):
        ...
        self._pred_states_a = None
        self._new_pred_states = None
        self.aug = False
        self.max_future = 0
    ...
    def new_action(self, action):
        ...
        if rgc_return == 1:
            ...
            self.robot_states.rcg_status = -1
        self._new_pred_states = self.RGC.pred_states
    ...
    def ml_states(self):
        self.robot_states.ankle_pos, self.robot_states.ankle_vel = self._write_wrt(self.mdl.HT_ankle[0:3, 3],
                                                                                   self.robot_states.ankle_pos)

        self.robot_states.toe_pos, self.robot_states.toe_vel = self._write_wrt(self.mdl.HT_toe[0:3, 3],
                                                                               self.robot_states.toe_pos)

        self.robot_states.heel_pos, self.robot_states.heel_vel = self._write_wrt(self.mdl.HT_heel[0:3, 3],
                                                                                 self.robot_states.heel_pos)

        self.robot_states.knee_pos, self.robot_states.knee_vel = self._write_wrt(self.mdl.HT_knee[0:3, 3],
                                                                                 self.robot_states.knee_pos)
    ...
    def set_pred_states_dim(self, _rows, _cols, augmented=False):
        self.aug = augmented
        self.max_future = _cols
        self.n_pred_states = _rows
        if self.aug:
            self._pred_states_a = np.zeros((22, _cols))  # hard code
            self._pred_states = np.zeros((_rows, _cols))
            self.pred_states = np.zeros((22, _cols))
        else:
            self._pred_states = np.zeros((_rows, _cols))
    ...
    def update_pred_states(self):
        for index in range(self.max_future):
            start = index * self.n_pred_states
            end = start + self.n_pred_states
            self._pred_states[:, index] = self._new_pred_states[start:end]

        # No augmentation: use raw predicted states directly
        if not self.aug:
            self.pred_states = self._pred_states.copy(order="C")
            return

        # Fill augmented structure
        self._pred_states_a[0:6, :] = self._pred_states[0:6, :]  # [r, dr, th, dth]

        # Predict b, db
        b, db = self._compute_pred_base()
        self._pred_states_a[6:8, :] = b
        self._pred_states_a[8:10, :] = db

        # Copy joint predicted states [q, dq, qr]
        self._pred_states_a[10:19, :] = self._pred_states[6:15, :]

        # Predict tau
        self._pred_states_a[19:, :] = self._update_pred_tau()

        # Final output
        self.pred_states = self._pred_states_a.copy(order="C")

    def _update_pred_tau(self):
        tau = np.zeros((3, self.max_future))
        for index in range(self.max_future):
            qr = self._pred_states[12:15, index]  # shape (3,)
            q = self._pred_states[6:9, index]
            dq = self._pred_states[9:12, index]
            tau[:, index] = self.Kp @ (qr - q) - self.Kd @ dq
        return tau

    def _compute_pred_base(self):
        J = np.zeros((3, 3))
        db = np.zeros((2, self.max_future))
        b = np.zeros((2, self.max_future))

        for index in range(self.max_future):

            q = self.pred_states[6:9, index]
            dq = self.pred_states[9:12, index]

            self.mdl.update_robot_states(q, dq)
            sin_vals = self.mdl.sin_vals
            cos_vals = self.mdl.cos_vals

            if self.robot_states.heel_cont and self.robot_states.toe_cont:

                # Construct simplified Jacobian
                J[0, 0] = 0.45 * cos_vals[0] - 0.26 * sin_vals[2] + 0.05 * cos_vals[2] + 0.50 * cos_vals[1]
                J[0, 1] = -0.26 * sin_vals[2] + 0.05 * cos_vals[2] + 0.50 * cos_vals[1]
                J[0, 2] = -0.26 * sin_vals[2] + 0.05 * cos_vals[2]
                J[2, 0] = 0.26 * cos_vals[2] + 0.05 * sin_vals[2] + 0.45 * sin_vals[0] + 0.50 * sin_vals[1]
                J[2, 1] = 0.26 * cos_vals[2] + 0.05 * sin_vals[2] + 0.50 * sin_vals[1]
                J[2, 2] = 0.26 * cos_vals[2] + 0.05 * sin_vals[2]

                db_full = J @ dq  # shape (3,)
                db[0, index] = db_full[0]  # ẋ
                db[1, index] = db_full[2]  # ż

                if index == 0:
                    b[:, index] = self.robot_states.b_pos[:, 0] + self.rgc_dt * db[:, index]
                else:
                    b[:, index] = b[:, index - 1] + self.rgc_dt * db[:, index]
            else:
                if index == 0:
                    db[:, index] = (self.robot_states.b_vel[:, 0] + self.rgc_dt * np.array([0, -9.81]))
                    b[:, index] = self.robot_states.b_pos[:, 0] + self.rgc_dt * db[:, index]
                else:
                    db[:, index] = db[:, index - 1] + self.rgc_dt * np.array([0, -9.81])
                    b[:, index] = b[:, index - 1] + self.rgc_dt * db[:, index]
