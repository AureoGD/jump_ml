...
class ModelMatrices:
    ...
    def update_com_pos(self):
        return ((self.HT_com0[0:3, 3] * self.m0 + self.HT_com1[0:3, 3] * self.m1 + self.HT_com2[0:3, 3] * self.m2 +
                 self.HT_com3[0:3, 3] * self.m3) / self.m).reshape((3, 1))
    ...
    def update_inertia_tensor(self):
        ...
        for idx in range(len(self.masses)):
            ...
            Ib += rot @ self.Inertia[idx] @ rot.transpose() + (self.masses[idx] * np.dot(trans, trans) * np.eye(3) -
