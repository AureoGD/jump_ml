...
OptProblem0::OptProblem0(ModelMatrices *Robot) : RobotMtx(Robot)
{
    ...
    // q, rx, grf_toe, grf_heel
    this->C_cons.resize(10, 13);
    this->C_cons.setZero();

    this->C_consV.resize(10, 1);
    this->C_consV.setZero();

    // predict states of interess  |r, dr, th, dth, q, dq, qr|
    this->C_st.resize(15, 13);
    this->C_st.setZero();

    this->D_st.resize(15, 3);
    this->D_st.setZero();
...
OptProblem0::~OptProblem0()
{
}

void OptProblem0::UpdateModelConstants()
{
 ...
 */
    ...
    // to the ground and maing the angular velcoity equal zero. Therefor ref = [0;0], the same value of the inicialization
    this->ref(0, 0) = 0.1;
    ...
    this->C_cons.block(7, 3, 3, 3) = -Kp * Eigen::MatrixXd::Identity(3, 3);
    this->C_cons.block(7, 10, 3, 3) = Kp * Eigen::MatrixXd::Identity(3, 3);
    ...
    Ub.resize(10, 1);
    Ub << RobotMtx->qU, 0, 0, OsqpEigen::INFTY, -g * 2.5 * RobotMtx->m, 0, OsqpEigen::INFTY, -g * 2.5 * RobotMtx->m;

    Lb.resize(10, 1);
    Lb << RobotMtx->qL, 0, -OsqpEigen::INFTY, 0, -g * 0.5 * RobotMtx->m, -OsqpEigen::INFTY, 0, -g * 0.5 * RobotMtx->m;
    ...
    this->C_st.block(0, 6, 2, 2) = Eigen::MatrixXd::Identity(2, 2);  // r 0 1
    this->C_st.block(2, 0, 2, 2) = Eigen::MatrixXd::Identity(2, 2);  // dr 2 3
    this->C_st(4, 8) = 1;                                            // th 4
    this->C_st(5, 2) = 1;                                            // th 5
    this->C_st.block(6, 3, 3, 3) = Eigen::MatrixXd::Identity(3, 3);  // q 6 7 8
    this->C_st.block(9, 0, 3, 3) = Eigen::MatrixXd::Identity(3, 3);  // dq 9 10 11
    this->C_st.block(12, 0, 3, 3) = Eigen::MatrixXd::Identity(3, 3); // qr 12 13 14

    this->D_st.block(12, 0, 3, 3) = Eigen::MatrixXd::Identity(3, 3); // qr 12 13 14
}

void OptProblem0::UpdateDynamicModel()
{
...
void OptProblem0::DefineConstraintMtxs()
{
    //
    ...
    this->Phi_cons.block(4, 0, 3, this->nxa) = -GRF_mtx * (Jcs.transpose()).inverse() * this->C_cons.block(4, 0, 3, this->nxa);
    this->aux_cons.block(4, 0, 3, this->nu) = -Kp * GRF_mtx * (Jcs.transpose()).inverse();

    auto J_heel = roty * RobotMtx->J_heel;

    Jcs << J_heel(0, 0), J_heel(0, 1), J_heel(0, 2), J_heel(2, 0), J_heel(2, 1), J_heel(2, 2), 1, 1, 1;

    this->Phi_cons.block(7, 0, 3, this->nxa) = -GRF_mtx * (Jcs.transpose()).inverse() * this->C_cons.block(7, 0, 3, this->nxa);
    this->aux_cons.block(7, 0, 3, this->nu) = -Kp * GRF_mtx * (Jcs.transpose()).inverse();

    this->ref(1, 0) = (this->Ucv_var(0, 0) / 2 + this->Lcv_var(0, 0) / 2);
    this->UpdateReferences(this->ref);
}