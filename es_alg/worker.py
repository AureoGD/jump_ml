...
def worker_init(config: Dict[str, Any], switch_rule_kwargs: Dict[str, Any]):
    ...
    sim_instance = Individual(individual_id=process_id, config=config)
    ...
    evaluator_kwargs = config.get('step_eval_kwargs',
                                  {})  # Get kwargs for StepEvaluator
...
def run_worker_task(task_args: Tuple[int, np.ndarray]) -> Tuple[int, float]:
    ...
    try:
        ...
        state_dict = unflatten_parameters_to_state_dict(
            nn_params_flat, switch_rule)
        ...
        sim_instance.reset()
        episode_state_history.append(sim_instance.robot.robot_states.copy())
        current_observation = evaluator.get_current_observation()
        max_steps = sim_instance.config.get('environment',
                                            {}).get("max_steps_per_episode",
                                                    1000)
        step = 0
        evaluator.reset()
        for step in range(max_steps):
            ...
            episode_state_history.append(step_history[-1])

            _, done, _ = evaluator.evaluate()
        ...
        fitness = evaluator.total_reward
